...
def get_price_running_instances():
    ...
    for region in regions:
        ...
        for instance in running_ec2_instances:
            ...
            print("{}: {} {} {}".format(aws_regions[region_name.lower()], instance_name, instance_type, launch_time))
            priceph = get_instance_pricing(instance_type, aws_regions[region_name.lower()])
            total_price = float(priceph.get('USD', 0)) * total_time_in_hours(launch_time)
            monthly_price = float(priceph.get('USD', 0)) * monthly_used_in_hours()
            print(" * Total accumulated price in USD: {0:.2f}".format(total_price))
            print(" * Monthly charged price in USD: {0:.2f}".format(monthly_price))
            estimated_monthly_price += monthly_price
            accumulated_total_price += total_price
    print("** Total monthly price for all instances in USD: {0:.2f}".format(estimated_monthly_price))
    print("** Total accumulated price for all instances in USD: {0:.2f}".format(accumulated_total_price))
...
def get_instance_pricing(instance_type, region_name):
    ...
    price_per_hours = price_tree.execute("$.terms.OnDemand.'{}'.priceDimensions.'{}'.pricePerUnit".format(sku, rateCode))
