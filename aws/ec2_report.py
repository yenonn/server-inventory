...
def get_running_ec2_instances():
    ...
    for region in regions:
        ...
        for instance in running_instances:
            ...
            launch_time_hour = total_time_in_hour(
                instance.launch_time.strftime("%Y-%m-%d %H:%M:%S"))
            monthly_time_hour = monthly_time_in_hour(
                instance.launch_time.strftime("%Y-%m-%d %H:%M:%S"))
            instanceinfo = []
...
def get_running_rds_instances():
    ...
    for region in regions:
        ...
        running_rds_instances = rds_client.describe_db_instances()[
            'DBInstances']
        for db in running_rds_instances:
            ...
            launch_time_hour = total_time_in_hour(
                db.get('InstanceCreateTime').strftime("%Y-%m-%d %H:%M:%S"))
            monthly_time_hour = monthly_time_in_hour(
                db.get('InstanceCreateTime').strftime("%Y-%m-%d %H:%M:%S"))
            instanceinfo = []
            instanceinfo.append(region_name.upper())
...
def monthly_time_in_hour(launch_time):
    ...
    first_day_of_month_datetime = datetime.strptime(
        first_day_of_month, datetimeFormat)
