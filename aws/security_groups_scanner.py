  ...
  for region in regions:
    ...
    for instance in running_instances:
      ...
      for sg in instance.security_groups:
        ...
        print("  + {}({}) with key:{} is using security groups: {}({})".format(instance_name, instance_state, instance_key_name, security_groupname, security_groupid))
    ...
    for sg in boto3.client('ec2', region_name=region_name).describe_security_groups()['SecurityGroups']:
      ...
      if security_groupid in membership.values():
        ...
        if security_groupname == "default":
          print("  - {}({}): skipped unused".format(security_groupname, security_groupid))
        else:
          print("  - {}({}): Unused".format(security_groupname, security_groupid))
      ...
      for sg_item in sg['IpPermissions']:
        if sg_item['IpProtocol'] == 'tcp':
          ...
          if sg_item_from_port == 0 and sg_item_to_port > 0 and len(sg_item_ip_range):
            for cidr_ip in sg_item_ip_range:
              if '0.0.0.0/0' in cidr_ip.values():
                hazard_sg[security_groupid] = security_groupname
                print("    (!!) WARNING Port TCP ({}-{}): {}".format(sg_item_from_port, sg_item_to_port, sg_item_ip_range))
              else:
                print("    (*) Port TCP ({}-{}): {}".format(sg_item_from_port, sg_item_to_port, sg_item_ip_range))
          else:
            print("    (*) Port TCP ({}-{}): {}".format(sg_item_from_port, sg_item_to_port, sg_item_ip_range))
